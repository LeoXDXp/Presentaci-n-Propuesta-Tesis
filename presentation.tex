...
\note[itemize]{
    ...
    \item El primer grupo se basa en el procesamiento de im\'agenes de bajo nivel, logrando obtener informaci\'on de caracter geometrico o topol\'ogico, enfocandose en el an\'alisis a nivel de p\'ixeles. La principal critica a estos métodos radica en la tendencia a un alto número de parámetros a configurar.
    
    \item El segundo grupo de métodos, adem\'as de utilizar la imagen inicial aplicando filtros de bajo nivel, se basa en la optimizaci\'on con el propósito de obtener informaci\'on adicional a la que se obtiene con el primer grupo. En particular, el uso de funciones de optimización responde a la necesidad de analizar combinaciones de opciones para construir una representación de una red de filamentos, o para la individualización de estos. Así, la información adicional obtenida representa caracter\'isticas cuantitativas que describen el comportamiento de los filamentos.
    
    \item Del 2do grupo de métodos, destacamos la investigación que da pie a la herramienta llamada DeFiNe, debido a que es la primera que realiza individualizaci\'on de filamentos.
    }
...
\small
    ...
    Este subconjunto se denomina P' es posible de obtener a partir de las heurísticas basadas en Randon Minimum Spanning Tree (RMST) o en Breath First Search.
...
\note[itemize]{
    ...
    \item A modo de resumen, se puede indicar que los m\'etodos basados en procesamiento de imágenes de bajo nivel tienen un alto n\'umero de par\'ametros, mientras que los basados en optimizaci\'on pueden tener un alto costo computacional
...
\note[itemize]{
    ...
    \item Identificar la ponderaci\'on de propiedades que entregue mejores resultados para grafos que representen casos espec\'ificos de filamentos como en una neurona y los microt\'ubulos en plantas
    \item Evaluar t\'ecnicas que usan s\'olo herramientas de procesamiento de im\'agenes de bajo nivel, basada en poblaci\'on de p\'ixeles, o herramientas que utilizan un m\'etodo derivado de contornos activos.
}
