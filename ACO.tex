...
\note[itemize]{
    ...
    \item El primer criterio busca asignar aristas que esten en el borde del grafo, como la arista A1.
...
\note[itemize]{
   \small
    ...
    \item y las soluciones que requieren de mayor evaluaci\'on dado que con los criterios aplicados durante la construcci\'on no pueden ser desechados ni aceptados. Estas son las de calidad intermedia.
    \item Como se observa en la presentaci\'on, el par\'ametro Max Score es el que define los rangos de calidad.
    
    \item (Respirar) Luego, las soluciones de calidad intermedia y de buena calidad pasan a las siguientes etapas, dado que se busca maximizar la funci\'on objetivo
}
...
\note[itemize]{
    ...
    \item Una arista con un tau penalizado por pertenecer a uno o m\'as caminos de mala calidad, puede bloquear otros caminos, dado que el valor de tau no guarda relaci\'on con las otras aristas que conforman el camino de mala calidad que caus\'o la penalizaci\'on. As\'i, las soluciones que pudiesen pasar por esa arista se ven limitadas.
...
\begin{frame}{Anti-feromonas SAP dependientes del camino previo (OE 1)}
    \centering
    \includegraphics[scale=0.50]{Pictures/ACO-ant-ferom-penalize-seg.png}
    \begin{itemize}
...
\note[itemize]{
    \item La anti-feromona se aplica sobre hormigas que al finalizar su recorrido entregan una soluci\'on cuya calidad sea intermedia, dado que las hormigas con soluciones cuya calidad es inferior a intermedia, que son las que no cumplen con la calidad m\'inima, ya han sido desechadas, mientras que las de buena calidad ya han sido seleccionadas como soluciones factibles.
...
\centering
    ...
    \begin{itemize}
        \item Se definen $\Vec{p} = v^{a}_1 - mc^{a}$ y $\Vec{q} = mc^{a} - v^{a}_5$.
